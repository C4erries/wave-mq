...
- Путь репликации (leader → follower) — реализованы бинарный клиент (`BinaryReplicator`) и PartitionReplicator (WAL‑sink + отчёт о прогрессе в ISR). RF>1 считается поддержанным режимом: клиенты пишут/читают на лидера (follower отвечает `ErrNotLeader` / HTTP 409 с `leaderBrokerID`), метаданные дают расклад leader/replicas для роутинга, метрики (`wavemq_replication_lag_offsets`, `wavemq_replication_applied_total`) показывают лаг и применённые записи на репликах.
...
- Маршрутизация клиентов по лидерам (реализовано в API):
  - клиенты узнают лидера партиции через бинарный Metadata‑запрос и HTTP (`/api/topics`, `/api/cluster`);
  - produce/fetch нужно отправлять на лидера, follower отвечает `ErrNotLeader` (HTTP 409) с `leaderBrokerID` для роутинга;
  - UI/метрики показывают роли/ISR и лаги репликации (`wavemq_replication_lag_offsets`, `wavemq_replication_applied_total`).
