...
- Replication path (leader → follower) — binary client (`BinaryReplicator`) and partition replicator (`PartitionReplicator` + WAL sink + ISR reporting) are implemented and can be enabled via `-replication`. RF>1 is supported at the protocol/metadata level; replication is opt-in and can be rolled out gradually.
...
- Leader-based client routing (design in place):
  - clients can discover leaders via the binary Metadata API and HTTP `/api/cluster`;
  - UI shows multiple brokers, their roles, ISR and high-watermarks on a dedicated cluster page.
